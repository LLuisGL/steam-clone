...
<body>
    ...
    <div class="h-screen bg-[#1a293a] flex flex-col gap-4 justify-center items-center">
        @if($juegos->isEmpty())
            <h1 class="text-4xl text-white opacity-70">Actualmente no hay juegos:(</h1>
        @else
                ...
                @foreach($juegos as $juego)
                    ...
                    <div class="flex gap-4 justify-end">
                        <div class="flex justify-end  mt-[5px]">
                            <form action="{{ route('juegos.editar', $juego->id)}}" method="GET">
                                @csrf
                                <input type="hidden" name="id_juego" value="{{ $juego->id }}">
                                <button type="submit" class="bg-orange-600 text-[#d2efa9] text-sm px-4 py-1 hover:bg-red-700 transition">
                                    Editar
                                </button>
                            </form>
                        </div>
